...
<section class="Psection MT20">
    ...
    <article class="U-article Overflow">
        <!--user order list-->
        <section>
            <form action="${path}/MerchantCenter/orders/list" method="post">
                <table>
            ...
            <table class="Myorder">
                ...
                <c:forEach items="${ordersList}" var="order">
                    <tr>
                        ...
                        <td><fmt:formatDate value="${order.regTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                ...
                <c:if test="${page.dataCount != 0}">
                    <c:if test="${empty param.keywords}" var="flag">
                        <c:forEach begin="0" end="${page.pageCount-1}" varStatus="stat">
                            <a href="${path}/MerchantCenter/orders/list?pageNum=${stat.count}" class="${stat.count eq page.pageNum?'current':''}">${stat.count}</a>
                        </c:forEach>
                    </c:if>
                    <c:if test="${not flag}">
                        <c:forEach begin="0" end="${page.pageCount-1}" varStatus="stat">
                            <a href="${path}/MerchantCenter/orders/showList?pageNum=${stat.count}&keywords=${requestScope.keywords}" class="${stat.count eq page.pageNum?'current':''}">${stat.count}</a>
                        </c:forEach>
                    </c:if>
                </c:if>
            </div>
