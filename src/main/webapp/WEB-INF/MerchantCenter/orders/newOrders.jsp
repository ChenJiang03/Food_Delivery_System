            ...
            <table class="Myorder">
                ...
                <c:forEach items="${ordersList}" var="order">
                    <tr>
                        ...
                        <td><a href="${path}/MerchantCenter/orders/ordersInfo?id=${order.id}">订单详情</a> | <a href="${path}/MerchantCenter/orders/accept?id=${order.id}">接单</a> |<a href="${path}/MerchantCenter/orders/cancel?id=${order.id}">取消订单</a></td>
                    </tr>
                </c:forEach>
            </table>
            <div class="list-page">
                ...
                <c:if test="${page.dataCount != 0}">
                    <c:forEach begin="0" end="${page.pageCount-1}" varStatus="stat">
                        <a href="${path}/MerchantCenter/orders/newOrders?pageNum=${stat.count}" class="${stat.count eq page.pageNum?'current':''}">${stat.count}</a>
                    </c:forEach>
                </c:if>
            </div>
        </section>
