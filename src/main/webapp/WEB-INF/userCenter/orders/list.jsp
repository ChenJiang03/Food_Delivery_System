...
<section class="Psection MT20">
    ...
    <article class="U-article Overflow">
        <!--user order list-->
        <section>
            <form action="${path}/userCenter/orders/list" method="post">
                <table>
            ...
            <table class="Myorder">
                <th class="Font14 FontW"><a class="Font14 FontW" href="${path}/userCenter/orders/list">全部订单</a></th>
                <th class="Font14 FontW"><a class="Font14 FontW" href="${path}/userCenter/orders/orderStatus?order_status=0">未送达订单</a></th>
                <th class="Font14 FontW"><a class="Font14 FontW" href="${path}/userCenter/orders/orderStatus?order_status=1">已完成订单</a></th>
                <th class="Font14 FontW"><a class="Font14 FontW" href="${path}/userCenter/orders/orderStatus?order_status=2">已取消订单</a></th>
            </table>
            <table class="Myorder">
                ...
                <c:forEach items="${ordersList}" var="order">
                    <tr>
                        ...
                        <td><a href="${path}/userCenter/orders/ordersInfo?id=${order.id}">订单详情</a>  <a href="${path}/userCenter/orders/cancel?id=${order.id}">${order.orderStatus eq 0? "| 取消订单":""}</a></td>
                    </tr>
                </c:forEach>
            </table>
            <div class="list-page">
                ...
                <c:if test="${page.dataCount != 0}">
                    <c:if test="${empty param.keywords}" var="flag">
                        <c:forEach begin="0" end="${page.pageCount-1}" varStatus="stat">
                            <a href="${path}/userCenter/orders/list?pageNum=${stat.count}" class="${stat.count eq page.pageNum?'current':''}">${stat.count}</a>
                        </c:forEach>
                    </c:if>
                    <c:if test="${not flag}">
                        <c:forEach begin="0" end="${page.pageCount-1}" varStatus="stat">
                            <a href="${path}/userCenter/orders/showList?pageNum=${stat.count}&keywords=${requestScope.keywords}" class="${stat.count eq page.pageNum?'current':''}">${stat.count}</a>
                        </c:forEach>
                    </c:if>
                </c:if>
            </div>
